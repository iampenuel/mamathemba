...
import {
  AFRICA_PILOT_WARNING,
  formatCapability,
  formatCommaSeparated,
  formatSavedAt,
  formatSubmittedFactValue,
  formatTravelDetail,
  formatTravelSummary,
  getBloodPressure,
  getFacilityBasisLabel,
  getFacilityDataBasis,
  getFacilityEvidenceText,
  getFacilityKey,
  getSelectedFacilityBadge,
  hasCuratedClinicalMetadata,
  normalizeToken,
  safeFacilityText,
...
function ReviewWorkspace({ reviewData }: { reviewData: ReviewData }) {
  ...
  const africaBoundaryMessage = reviewData.missing_information.find((item) =>
    item.includes(AFRICA_PILOT_WARNING)
  );
